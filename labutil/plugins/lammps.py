...
def write_lammps_data(struc, runpath, triclinic=False):
    ...
    if triclinic:
        datatxt += "{} {} {} xy xz yz\n".format(
            struc.cell[0][1], struc.cell[0][2], struc.cell[1][2]
        )
    ...
    for index, site in enumerate(struc.positions):
        datatxt += "{} {} {:1.5f} {:1.5f} {:1.5f} \n".format(
            index + 1, struc.species[site[0]]["kind"], *site[1]
        )
...
def write_lammps_input(datafile, runpath, rdffile, intemplate, inparam, potential):
    ...
    subst = {
        "DATAINPUT": datafile.path,
        "POTENTIAL": ppath,
        "DUMPFILE": dumpfile.path,
        "RDFFILE": rdffile.path,
    }
...
def lammps_run(struc, runpath, intemplate, potential, inparam, ncpu=1, triclinic=False):
    ...
    lammps_command = "mpirun -n {} {} -in {} -log {} > {}".format(
        ncpu, lammps_code.path, infile.path, logfile.path, outfile.path
    )
